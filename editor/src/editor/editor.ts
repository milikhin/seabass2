...
export default class Editor extends EventTarget {
  ...
  _onKeyPress = (evt: KeyboardEvent): void => {
    ...
    //   }
    // }));
    /* Some keys are handled twice on SfOS, disable redundant keypress handler */
    const duplicatedKeyCodes = ['Enter', 'Backspace', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown']
    if (duplicatedKeyCodes.includes(evt.key)) {
      evt.preventDefault()
    }
  }
